...
import { getChunkPlanes } from "./chunkPlanes";
import { getTexture } from "./textureManager";
import type { CameraGridState, ChunkData, PlaneData } from "./types";
...
const getChunkUpdateThrottleMs = (isZooming: boolean, zoomSpeed: number) =>
  zoomSpeed > 1.0 ? 500 : isZooming ? 400 : 100;

const shouldThrottleUpdate = (last: number, throttleMs: number, now: number) => now - last >= throttleMs;
...
function MediaPlane({
  ...
  React.useEffect(() => {
    ...
    const tex = getTexture(media.url, () => {
      state.ready = true;
      setIsReady(true);
    });
    setTexture(tex);
...
function SceneController({ media }: { media: LandingMedia[] }) {
  ...
  useFrame(() => {
    ...
    // Reduced motion: freeze the idle mouse-parallax drift (drag/scroll still work).
    if (reducedMotion.current) {
      s.drift.x = 0;
      s.drift.y = 0;
    } else if (s.isDragging) {
      // keep drift frozen during drag
  ...
  React.useEffect(() => {
    ...
    setChunks(
      CHUNK_OFFSETS.map((o) => ({ key: `${o.dx},${o.dy},${o.dz}`, cx: o.dx, cy: o.dy, cz: o.dz }))
    );
  }, [camera]);
...
export function InfiniteCanvasScene({ media }: { media: LandingMedia[] }) {
  const isTouchDevice = useIsTouchDevice();
    ...
    <KeyboardControls map={KEYBOARD_MAP}>
      <div style={{ width: "100%", height: "100%" }}>
        ...
        >
          ...
          <fog attach="fog" args={["#f8f7f4", 120, 320]} />
